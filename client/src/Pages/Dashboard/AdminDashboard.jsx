...
function AdminDashboard() {
    ...
    const { allUsersCount = 0, subscribedCount = 0 } = useSelector((state) => state.stat);
    const { allPayments, monthlySalesRecord } = useSelector((state) => state.razorpay);
    const myCourses = useSelector((state) => state?.course?.courseData);

    const revenue = (allPayments?.count ?? 0) * 499;
    const monthlySales = Array.isArray(monthlySalesRecord) && monthlySalesRecord.length === 12 
        ? monthlySalesRecord 
        : new Array(12).fill(0);
    ...
    useEffect(() => {
        (async () => {
          const stats = await dispatch(getStatsData());
          console.log("Stats data:", stats);
      
          const payments = await dispatch(getPaymentRecord());
